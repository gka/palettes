<script>
    ...
    $: stepsLeft = colors.length ? chroma.scale(bezier && colors.length>1 && colors.length<=5 ? chroma.bezier(genColors) : genColors)
        .correctLightness(correctLightness)
        .colors(numColorsLeft) : [];

    $: stepsRight = diverging && colors2.length ? chroma.scale(bezier&& colors2.length>1 &&colors2.length<=5 ? chroma.bezier(genColors2) : genColors2)
        .correctLightness(correctLightness)
